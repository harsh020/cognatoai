...
function JobDetails({ job, ref }) {
    ...
    const filteredStages = useMemo(() => {
        if(stages && job) {
            return filterStages(stages.data.filter(s => job.stages.includes(s.id)));
        } return [];
    }, [stages, job]);
                ...
                <CardContent className="p-4 space-y-4"> {/* Content padding and spacing */}
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2"> {/* Use flex-wrap for badges */}
                            {job.stages && filteredStages.length > 0 && stages ? (
                                filteredStages.map((skill, index) => {
                              ...
                              <p className="text-sm text-muted-foreground">Something went wrong we can&apos;t seem to fetch skills.</p>
                            ) : (
...
export default function Job(props) {
    ...
    return (
        <div className='flex flex-col h-fit w-full overflow-y-auto scrollbar-thin p-0 gap-4'>
            {
                (loading || stageLoading) ? (
                  ...
                  jobs.count ? (
                    jobs.results.map((job, index) => (
                      <JobDetails key={index} ref={params.get('id') === job.id ? selectedJobRef : null} job={job} />
                    ))
                  ) : (
                    <NoData className='h-[calc(100vh-5.2rem)]' text='No job template created.' />
                  )
                ) : (
                    <NoData className='h-[calc(100vh-5.2rem)]' text='Something went wrong. Please try again later.' />
                )
            }
        </div>
    );
