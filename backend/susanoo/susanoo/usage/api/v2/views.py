...
from susanoo.usage.models import Usage, UsageV2


def _get_linguistics_cost(start, end, tokens):
    response = requests.request(
        method='GET',
        url=f'{settings.LINGUISTICS_URL}/api/v1/linguistics/usage',
        params={
            'start': start,
            'end': end,
            'tokens': tokens
...
class CostView(generics.GenericAPIView):
    ...
    def get(self, request, id=None):
        ...
        interview = InterviewV2.objects.get(id=interview_id)
        ...
        usages = UsageV2.objects.filter(interview__id=interview_id)
        ...
        tokens = 0
        for message in MessageV2.objects.filter(interview=interview, type=MessageType.AI).all():
        ...
        for linguistics_usage in linguistics_usages.get('usage'):
            total_cost += decimal.Decimal(linguistics_usage.get('cost'))
            trace_per_model[linguistics_usage.get('model')] = {
                'cost': linguistics_usage.get('cost')
            }
